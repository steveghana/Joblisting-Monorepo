...
const RoleTabs = (props: IRoleTabs) => {
  ...
  const tabs = props.isExternal ? roleTabs.filter(tab => tab.value !== 'applicants') : roleTabs;
        ...
        <SubCard>
          <Container maxWidth="lg">
            ...
            >
              ...
              <Box sx={{ width: '100%' }} display={'flex'} gap={1} my={3} p={{ lg: 2, md: 1, sm: 0 }} alignItems={'center'} flexWrap={'wrap'}>
              ...
              <Grid item xs={12}>
                {currentTab === 'overview' && <RoleDetails setCurrentTab={setCurrentTab} role={role} />}
                {currentTab === 'jobs' && <JobsPage job={role.jobs} client={role.client} roleId={role.id} />}
              </Grid>
            </Grid>
          </Container>
        </SubCard>
