...
export class SavannatechDb1702412522538 implements MigrationInterface {
    name = 'SavannatechDb1702412522538'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE "interviews" ("createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), "id" uuid NOT NULL DEFAULT uuid_generate_v4(), "eventType" character varying NOT NULL, "eventOption" character varying NOT NULL, "description" character varying NOT NULL, "eventLInk" character varying NOT NULL, "starttime" character varying NOT NULL, "endtime" character varying NOT NULL, "startDate" TIMESTAMP NOT NULL, "endDate" TIMESTAMP NOT NULL, "status" character varying NOT NULL, "role_id" uuid, CONSTRAINT "PK_fd41af1f96d698fa33c2f070f47" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "jobs" ("createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), "id" uuid NOT NULL DEFAULT uuid_generate_v4(), "description" character varying NOT NULL, "roleType" character varying NOT NULL DEFAULT 'Remote', "whenToStart" TIMESTAMP NOT NULL DEFAULT '"2023-12-12T20:22:05.165Z"', "employmentType" character varying NOT NULL, "country" character varying NOT NULL, "selectedSkills" text NOT NULL, "roleName" character varying NOT NULL, "jobType" character varying NOT NULL, "salary" character varying NOT NULL, "tasks" text NOT NULL, "roleCategory" character varying NOT NULL, "postedDate" TIMESTAMP NOT NULL, "role_id" uuid, "developer_id" uuid, CONSTRAINT "PK_cf0a6c42b72fcc7f7c237def345" PRIMARY KEY ("id"))`);
        ...
        await queryRunner.query(`CREATE TABLE "authToken" ("createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), "id" uuid NOT NULL, "isActive" boolean NOT NULL DEFAULT true, "lastUsed" TIMESTAMP NOT NULL DEFAULT '"2023-12-12T20:22:05.169Z"', "credentialTokenId" integer, "userEmail" character varying, CONSTRAINT "PK_eb8fa60b49ec49031fa0cab6d3e" PRIMARY KEY ("id"))`);
        ...
        await queryRunner.query(`CREATE TABLE "developer_interviews" ("interviewId" uuid NOT NULL, "developerId" uuid NOT NULL, CONSTRAINT "PK_e0600abdde46b8428c270c66eb2" PRIMARY KEY ("interviewId", "developerId"))`);
        await queryRunner.query(`CREATE INDEX "IDX_6ee408a963d59bff7ce9c132cf" ON "developer_interviews" ("interviewId") `);
        await queryRunner.query(`CREATE INDEX "IDX_4b72ad013f0b73fcbb6168f8af" ON "developer_interviews" ("developerId") `);
        ...
        await queryRunner.query(`ALTER TABLE "developer_interviews" ADD CONSTRAINT "FK_6ee408a963d59bff7ce9c132cff" FOREIGN KEY ("interviewId") REFERENCES "interviews"("id") ON DELETE CASCADE ON UPDATE CASCADE`);
        await queryRunner.query(`ALTER TABLE "developer_interviews" ADD CONSTRAINT "FK_4b72ad013f0b73fcbb6168f8af0" FOREIGN KEY ("developerId") REFERENCES "developer"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE "developer_interviews" DROP CONSTRAINT "FK_4b72ad013f0b73fcbb6168f8af0"`);
        await queryRunner.query(`ALTER TABLE "developer_interviews" DROP CONSTRAINT "FK_6ee408a963d59bff7ce9c132cff"`);
        ...
        await queryRunner.query(`DROP INDEX "public"."IDX_4b72ad013f0b73fcbb6168f8af"`);
        await queryRunner.query(`DROP INDEX "public"."IDX_6ee408a963d59bff7ce9c132cf"`);
        await queryRunner.query(`DROP TABLE "developer_interviews"`);
